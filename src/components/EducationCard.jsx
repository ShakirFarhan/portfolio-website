...
const EducationCard = ({ duration, type, degree, degreeType, course, schoolName, schoolLocation, cardType }) => {
  return (
    <div className={`w-[100%] bg-[#c2c2c2] rounded-[20px] h-[280px] sm:h-[250px] lg:h-[270px] p-4  lg:p-7 ${styles.hover__card}`}>
      <div className='flex items-center justify-between'>
        ...
        <div className={`flex items-center gap-x-3 bg-[var(--white-primary)] px-2 sm:px-5 py-[4px] sm:py-[6px] text-[13px] rounded-[50px] text-[var(--black-primary)] font-extrabold trackin-wider`}><img className='w-[20px] h-[20px]' src={graduationImg} />{type}</div>
      </div>
      <div className='flex items-center gap-x-4 my-3'  >
        {degree && <div className={`flex items-center gap-x-3 bg-[var(--white-primary)] px-2 sm:px-5 py-[4px] sm:py-[6px] text-[13px] rounded-[50px] text-[var(--black-primary)] font-bold`}>{degree}</div>}
        {degreeType && <div className={`flex items-center gap-x-3 bg-[var(--white-primary)] px-2 sm:px-5 py-[4px] sm:py-[6px] text-[13px] rounded-[50px] text-[var(--black-primary)] font-bold`}>{degreeType}</div>}
      </div>
      <div>
        <h2 className="text-[var(--black-primary)] text-[18px] lg:text-[20px] font-black mb-3">{course}</h2>
        <div className='flex items-center gap-x-4 mb-3'>
          <div className={`bg-[#fff] w-[40px] h-[40px] rounded-[50%] flex items-center justify-center`}>
